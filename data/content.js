  ...
  SP.data.content = {
    ...
    labels: {
      ...
      picker: {
        ...
        // มุมมองรวม (CR-13 หน้าจัดสรรเป้าหมายรายเดือน)
        all: 'ทั้งหมด',
    ...
    pages: {
      ...
      phasing: {
        ...
        // มุมมองรวม (CR-13): อ่านอย่างเดียว ผลรวมคำนวณทุกครั้ง (calc.aggregatePhasing) ไม่เก็บลง store
        aggregate: {
          readOnly: 'มุมมองรวม · เลือกหน่วยขายเพื่อแก้ไข',
          chartTitle: '{name} · เป้าหมายรายเดือน (ล้านบาท)',
          priorTip: 'รวมยอดขายปี {year} ของ {n} หน่วยขายในแผน',
          chartLabel: 'รูปแบบกราฟ',
          chartModes: { total: 'แท่งรวม', units: 'แยกตามหน่วยขาย', channels: 'แยกตาม Channel' },
          legendTotal: 'เป้าหมายรวม',
          legendPrior: 'ยอดขายปี {year}',
          legendTip: 'ไปที่มุมมองของ {name}',
          tipTotal: 'รวม',
          tipPrior: 'ยอดขายปี {year}',
          unitsRow: 'เป้าหมายรายหน่วยขาย ({n} หน่วย)',
          unitsToggle: 'แสดงหรือซ่อนเป้าหมายรายหน่วยขาย',
          incomplete: 'หน่วยขายที่ยังจัดสรรไม่ครบ: {n} หน่วย',
          complete: 'หน่วยขายจัดสรรครบทุกหน่วย',
          openUnit: 'เปิดจัดสรรเป้าหมายรายเดือนของ {name}',
          exportStatus: 'มุมมองรวม (อ่านอย่างเดียว)',
          exportSpec: {
            file: 'Phasing_{year}_{unit}_{date}', sheet: 'Phasing รวม',
            cols: { kind: 'รายการ', unit: 'หน่วยขาย', channel: 'Channel', total: 'รวมทั้งปี (บาท)', growth: 'การเติบโตเทียบปีก่อน (%)' },
            kinds: { target: 'เป้าหมาย Net Sales (บาท)', prior: 'ยอดขายปี {year} (บาท)' }
          }
        }
      },
      ...
      summary: {
        ...
        yearLines: {
          topDown: 'จัดสรรเป้าหมายประจำปี: {status}',
          topDownApproved: 'จัดสรรเป้าหมายประจำปี: อนุมัติแล้ว · {by} · {at}',
        ...
        approvalsTopDown: 'จัดสรรเป้าหมายประจำปี: อนุมัติแล้ว · {by} · {at}',
      ...
      aboutPrototype: {
        ...
        decisions: [
          ...
          { topic: 'ติดตามสถานะแยกจากรายงาน', decision: 'ขั้นที่ 4 มี 2 แท็บ: รายงานสรุปแผน (แท็บแรกและค่าเริ่มต้นเสมอ ประกาศและหลักฐานข้อตกลง — พิมพ์ได้) กับติดตามสถานะ (ตารางเดียว 1 แถวต่อหน่วยขาย รวมประเด็นที่ต้องดำเนินการ ผู้อนุมัติและวันที่อนุมัติ พร้อมประวัติ และปุ่มล็อก Baseline — ไม่พิมพ์)' },
          { topic: 'มุมมองรวมรายเดือน', decision: 'หน้าจัดสรรเป้าหมายรายเดือนดูผลรวมทั้ง Channel หรือทุก Channel ได้ แบบอ่านอย่างเดียว (แท่งซ้อนตามหน่วยขาย) การแก้ไขและส่งอนุมัติทำรายหน่วยขาย เพราะเป้าหมายและคงเหลือคิดต่อหน่วยขาย' },
          ...
          { topic: 'รายงานเป็นหลักฐาน', decision: 'ทุกครั้งที่ล็อก Baseline ได้รายงานฉบับใหม่ (2027-BL-01, 02 …) ตัวเลขทั้งหมดมาจาก Baseline ที่ล็อกไว้ พร้อมตารางผู้อนุมัติรายหน่วยขายและช่องลงนาม · ก่อนล็อกเป็นฉบับร่างมีลายน้ำทุกหน้า · ปลดล็อกได้โดย Sales Director พร้อมเหตุผล' }
        ],
        openQuestions: [

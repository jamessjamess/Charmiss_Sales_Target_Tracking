...
(function (SP) {
  ...
  function render(root, ctx) {
    ...
    function draw() {
      ...
      var filterCh = filter === 'all' ? null : calc.findById(channels, filter);
      var gpHead = filterCh && filterCh.gpLabel ? fill(page.gpColumn, { label: filterCh.gpLabel }) : cols.gp;
      ...
      root.appendChild(h('div', { class: 'master-toolbar' },
        ...
        editing ? h('button', { type: 'button', class: 'btn btn-sm', 'aria-expanded': addOpen ? 'true' : 'false', onClick: function () { addOpen = !addOpen; draw(); } },
          filterCh ? SP.core.components.fill(SP.data.content.labels.addUnitButton, { unit: filterCh.unitLabel }) : page.addButton) : null,
